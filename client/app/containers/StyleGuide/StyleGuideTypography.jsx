import React from 'react';

// components
import StyleGuideComponentTitle from '../../components/StyleGuideComponentTitle';
import Table from '../../components/Table';

export default class StyleGuideTypography extends React.Component {

  render = () => {

    let columns = [
      {
        header: 'Text Style',
        valueName: 'textStyle'
      },
      {
        header: 'CSS Rules',
        valueName: 'rules'
      }
    ];

    let rowObjects = [
      { textStyle: <div className="cf-display-1">Display 1</div>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 700</span>
          <span>font-size: 52px</span>
          <span>line-height: 1.3em/68px</span>
        </span>
      },
      { textStyle: <div className="cf-display-2">Display 2</div>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 300</span>
          <span>font-size: 44px</span>
          <span>line-height: 1.3em/57px</span>
        </span>
      },
      { textStyle: <h1>Heading 1</h1>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 700</span>
          <span>font-size: 34px</span>
          <span>line-height: 1.3em/44px</span>
        </span>
      },
      { textStyle: <h2>Heading 2</h2>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 700</span>
          <span>font-size: 24px</span>
          <span>line-height: 1.3em/31px</span>
        </span>
      },
      { textStyle: <h3>Heading 3</h3>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 700</span>
          <span>font-size: 19px</span>
          <span>line-height: 1.3em/25px</span>
        </span>
      },
      { textStyle: <h4>Heading 4</h4>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 700</span>
          <span>font-size: 15px</span>
          <span>line-height: 1.3em/20px</span>
        </span>
      },
      { textStyle: <h5>Heading 5</h5>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 400</span>
          <span>font-size: 13px</span>
          <span>line-height: 1.3em/17px</span>
        </span>
      },
      { textStyle: <p className="cf-lead-paragraph">Lead paragraph</p>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 300</span>
          <span>font-size: 22px</span>
          <span>line-height: 1.5em/33px</span>
        </span>
      },
      { textStyle: <p className="cf-body-copy">
            Body copy. A series of
            sentences together which
            make a paragraph.
          </p>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-weight: 400</span>
          <span>font-size: 17px</span>
          <span>line-height: 1.5em/26px</span>
        </span>
      },
      { textStyle: <p className="cf-italic-body-copy">
            Italic body copy. A series of
            sentences together which
            make a paragraph.
          </p>,
        rules: <span>
          <span>font-family: ‘Source Sans Pro’</span>
          <span>font-style: Italic</span>
          <span>font-weight: 400</span>
          <span>font-size: 17px</span>
          <span>line-height: 1.5em/26px</span>
        </span>
      }
    ];

    return <div>
      <StyleGuideComponentTitle
        title="Typography"
        id="typography"
        link="StyleGuideColors.jsx"
      />
      <Table
        className="cf-sg-typography"
        columns={columns}
        rowObjects={rowObjects}
        summary="Caseflow Typography" />
    </div>;
  };

  old_render = () => {
    return <div>
      <StyleGuideComponentTitle
        title="Typography"
        id="typography"
        link="StyleGuideColors.jsx"
      />

    <table className="cf-sg-typography">
        <caption>Text Styles</caption>
        <thead>
          <tr>
            <th>Text Style</th>
            <th>CSS Rules</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>
              <div className="cf-display-1">Display 1</div>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 700</span>
              <span>font-size: 52px</span>
              <span>line-height: 68px</span>
            </td>
          </tr>
          <tr>
            <td>
              <div className="cf-display-2" what>Display 2</div>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 300</span>
              <span>font-size: 44px</span>
              <span>line-height: 57px</span>
            </td>
          </tr>
          <tr>
            <td>
              <h1 className="cf-heading-1">Heading 1</h1>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 700</span>
              <span>font-size: 34px</span>
              <span>line-height: 44px</span>
            </td>
          </tr>
          <tr>
            <td>
              <h2 className="cf-heading-2">Heading 2</h2>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 700</span>
              <span>font-size: 24px</span>
              <span>line-height: 31px</span>
            </td>
          </tr>
          <tr>
            <td>
              <h3 className="cf-heading-3">Heading 3</h3>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 700</span>
              <span>font-size: 19px</span>
              <span>line-height: 25px</span>
            </td>
          </tr>
          <tr>
            <td>
              <h4 className="cf-heading-4">Heading 4</h4>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 700</span>
              <span>font-size: 15px</span>
              <span>line-height: 20px</span>
            </td>
          </tr>
          <tr>
            <td>
              <h5 className="cf-heading-5">HEADING 5</h5>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 400</span>
              <span>font-size: 13px</span>
              <span>line-height: 17px</span>
            </td>
          </tr>
          <tr>
            <td>
              <p className="cf-lead-paragraph">
                Lead paragraph
              </p>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 300</span>
              <span>font-size: 22px</span>
              <span>line-height: 33px</span>
            </td>
          </tr>
          <tr>
            <td>
              <p className="cf-body-copy">
                Body copy. A series of
                sentences together which
                make a paragraph.
              </p>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-weight: 400</span>
              <span>font-size: 17px</span>
              <span>line-height: 26px</span>
            </td>
          </tr>
          <tr>
            <td>
              <p className="cf-italic-body-copy">
                Italic body copy. A series of
                sentences together which
                make a paragraph.
              </p>
            </td>
            <td>
              <span>font-family: ‘Source Sans Pro’</span>
              <span>font-style: Italic</span>
              <span>font-weight: 400</span>
              <span>font-size: 17px</span>
              <span>line-height: 26px</span>
            </td>
          </tr>
        </tbody>
      </table>
    </div>;
  }
}
